# 50 · Running Classic today

Sections 37 through 49 are archaeology. This one is the practical summary: what to install in 2026, what
to build on, and what the twenty-year lineage actually leaves you.
...
4. Extract Classic 1.5.2 into GameData                  [section 39]
5. Copy TacoServer's Classic/ over it, overwriting      [section 49]
...
admin HUD (section 39). On an internet-reachable server that is remote code execution delegated to anyone
...
(section 48). No build step, no manifest, no cache; deleting a file removes a feature cleanly. Order with
...
**Take: optional rules as prefs, announced to the client.** Section 40. Keep the count small, default them
...
**Take: thresholds instead of settings, where population varies.** Section 49. A rule that is right at
sixteen players and wrong at four should be a number, not a decision.

**Avoid: generating script at boot.** Section 45's `evoPackage.cs`. If you must, regenerate
...
**Silent failure is the dominant defect mode.** Section 47's teamkill log ran wrong for a decade because
TorqueScript reads an undeclared local as `""` without complaint. Section 48's mission-type guard almost
certainly never fires, for a precedence reason that reads correctly in English. Section 45's generated
...
- [37 · Classic](../37-classic/README.md) — the lineage overview
- [39 · Classic 1.5.2](../39-classic-152/README.md) — the required base
- [49 · TacoServer in operation](../49-tacoserver-operation/README.md) — the recommended codebase

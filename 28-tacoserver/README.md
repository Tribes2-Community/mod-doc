...
| `$Host::ClassicLoad*` toggles | **All ten** survive intact (section 24) |
| `.ovl` files | **None** |
...
Evolution's own ancestor (section 25). A variable name has survived three codebases and twenty-two years.

`AntiLouExploitFixes.cs` is the thread from section 23 completing its journey: exploit fixes credited to
...
Evolution's problem was that packages cannot span files, so it generated one giant package (section 25).
...
`zz_Classic_client_v1.vl2` (section 22), applied to script load order rather than archive precedence.
...
Sections 25–27 traced a single logging line. TacoServer's treatment of the same function closes the arc.

Evolution logged inline inside the override, referencing an undeclared `%client` (section 27). TacoServer
...
and a glance at `logs/` settles it in a minute. The pattern is the same family as section 27's: a
...
- [29 · TacoServer in operation](../29-tacoserver-operation/README.md) — the feature set and running it
- [23 · Classic 1.5.2](../23-classic-152/README.md) — the required base
- [25 · Evolution Admin Mod](../25-evolution-admin-mod/README.md) — the architecture it discarded
- [27 · teratos' evoClassic](../27-teratos-evoclassic/README.md) — the bug this design eliminates

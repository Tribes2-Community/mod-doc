...
`EnableTeamBalanceNotify`) — the same "tell the client what ruleset it is in" discipline as section 40.
...
All ten `$Host::ClassicLoad*` toggles from section 40 are present and shipped in the prefs file, which is
...
(section 48); `NoSmurfs`, `GuidCheck`, `ClassicViralBanning`, `ClassicWhitelist`, `FloodProtectionEnabled`
...
Step 3 is the overlay from section 48 — order matters, and reversing steps 2 and 3 silently reverts
TacoServer to 1.5.2.
...
1.5.2 knew about (section 38). TacoServer adds no new script directories — everything lives in
...
is precisely why the lineage in section 48 had to be reconstructed by fingerprinting rather than read
from history.

## Related

- [48 · TacoServer](../48-tacoserver/README.md) — lineage and architecture
- [50 · Running Classic today](../50-running-classic-today/README.md) — choosing between these codebases
- [40 · The ruleset toggles](../40-classic-ruleset-toggles/README.md) — the `ClassicLoad*` prefs it inherits

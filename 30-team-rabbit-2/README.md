...
archives**, distinct from every other gametype in sections 22–29 — and the most elaborate single piece of
...
**A note on markers.** Like `DnDGame.cs` (section 31), these are third-party-authored files — but they
...
KineticPoet is the same person Classic's readme thanks for the CTF flag-collision fix (section 22) — one
...
change, which is the same "no economy" instinct Spawn CTF applies to its armour class (section 22),
...
`-43` against base's `-20` and Classic's `-26.9` (section 37) — more than double retail gravity, tuned
...
[31 · The base ruleset](../31-base-ruleset/README.md#the-armours) left in place as a reference point the
...
covered in sections 37–40 — but it treats the gametype as first-class rather than ignoring it.
...
integration work than a toggle requires, and it corroborates the KineticPoet credit in section 22: TR2's
developers and Classic's were reading and patching around each other's code.

Evolution Admin Mod's `.ovl` overlay (section 45) reaches TR2 too, with its own `TR2Game.ovl` overriding
...
Construction is (section 58): built on Tribes 2's engine, but not recognisably Tribes 2's ruleset once
you are inside a match.

## Related

- [22 · Capture the Flag](../22-capture-the-flag/README.md) — KineticPoet's other credited contribution, the flag-collision fix
- [31 · The base ruleset](../31-base-ruleset/README.md#the-armours) — the armour jet-force formula TR2's physics diverged from
- [37 · Classic](../37-classic/README.md) — the shadowing model TR2 sits outside of
- [45 · Evolution Admin Mod](../45-evolution-admin-mod/README.md) — `TR2Game.ovl`, the third codebase integrating this gametype
- [58 · The Construction Mod](../58-construction-mod/README.md) — the other total conversion in this handbook
